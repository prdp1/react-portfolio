...
const NavBar = () =>{
    ...
    return (
      <div className='flex justify-between items-center w-full h-20 bg-black text-white fixed'>
            <div>
                <h1 className='text-5xl font-signature ml-2'>papun</h1>
            </div>
            ...
            <ul className='hidden md:flex'>
                ...
                {links.map (({id,link}) =>(
                    ...
                    key={id} 
                    className='px-4
                    cursor-pointer capitalize font-medium text-gray-500 hover:scale-110 duration-200'>{link}</li>
                ))}
            ...
            {nav && (
                    ...
                    {links.map (({id,link}) =>(
                            <li
                            key={id} 
                            className='px-4 cursor-pointer capitalize py-6 text-4xl hover:scale-110 duration-200 '>{link}</li>
                        ))}
                    </ul>
            )}
